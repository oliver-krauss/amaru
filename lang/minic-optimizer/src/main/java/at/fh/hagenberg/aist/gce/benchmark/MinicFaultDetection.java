 ...
 */
public class MinicFaultDetection {
    ...
    private static final int GROW_CNT = 30;
...
//        printer.setOmitPatternsPerProblem(true);
        ...
        exceptions.forEachRemaining(x -> {
            if (!x.get(0).isNull()) {
                // trim to first line
                relevantExceptions.add(process(x.get(0).asString()));
            }
        });

        // print the exception matrix - can be commented out after first time
        TruffleOptimizationSolutionRepository truffleOptimizationSolutionRepository = ApplicationContextProvider.getCtx().getBean("truffleOptimizationSolutionRepository", TruffleOptimizationSolutionRepository.class);
        Iterable<TruffleOptimizationSolution> all = truffleOptimizationSolutionRepository.findAll();
        int matSize = relevantExceptions.size() + 1;
        int[][] occurences = new int[matSize][matSize];
        int[] asts = new int[matSize];
        int[] total = new int[matSize];
        int[] exclusive = new int[matSize];
        int[] nonexclusive = new int[matSize];
        List<String> exceptionIndex = new ArrayList<>(relevantExceptions);
//        exceptionIndex.add("java.lang.IllegalStateException");
//        exceptionIndex.add("java.lang.IllegalArgumentException");
//        exceptionIndex.add("An illegal reflective access operation has occurred");
//        exceptionIndex.add("com.oracle.truffle.api.dsl.UnsupportedSpecializationException");
//        exceptionIndex.add("java.util.concurrent.TimeoutException");
//        exceptionIndex.add("java.lang.ClassCastException");
//        exceptionIndex.add("FAILED TO SERIALIZE MESSAGE");
//        exceptionIndex.add("java.lang.NullPointerException");
//        exceptionIndex.add("java.lang.ArrayIndexOutOfBoundsException");
//        exceptionIndex.add("The worker crashed");
//        exceptionIndex.add("java.lang.RuntimeException");
//        exceptionIndex.add("java.lang.ArithmeticException");

        all.forEach(solution -> {
            int[] checked = new int[matSize];
            solution.getTestResults().forEach(r -> {
                if (r.getException() == null) {
                    if (checked[matSize - 1] == 0) {
                        asts[matSize -1] += 1;
                    }
                    total[matSize -1] += 1;
                    checked[matSize - 1] += 1;
                } else {
                    int pos = exceptionIndex.indexOf(process(r.getException()));
                    if (checked[pos] == 0) {
                        asts[pos] += 1;
                    }
                    total[pos] += 1;
            ...
            for (int i1 = 0; i1 < checked.length; i1++) {
                if (checked[i1] > 1) {
                    // NOTE OCCURENCES ONLY COUNTS UP IF IT IS CO-LOCATED WITH ITSELF!!!
                    occurences[i1][i1] += 1;
                }
                if (checked[i1] == solution.getTestResults().size()) {
                    exclusive[i1] += 1;
                }
                for (int i2 = i1 + 1; i2 < checked.length; i2++) {
                    if (checked[i1] > 0 && checked[i2] > 0) {
                        occurences[i1][i2] += 1;
                    }
                }
            }
        ...
        for (int i1 = 0; i1 < exclusive.length; i1++) {
            nonexclusive[i1] = asts[i1] - exclusive[i1];
        }

        System.out.println("Occurence Matrix ");
        System.out.println("TOTAL: " + Arrays.toString(total));
        System.out.println();
        System.out.println("Co-Located: " + Arrays.stream(nonexclusive).mapToObj(String::valueOf).collect(Collectors.joining(" & ")));
        System.out.println("EXCLUSIVE: " + Arrays.stream(exclusive).mapToObj(String::valueOf).collect(Collectors.joining(" & ")));
        System.out.println("TOTAL: " + Arrays.stream(asts).mapToObj(x -> "\\textbf{" + x + "}").collect(Collectors.joining(" & ")));
        System.out.println();
        for (int j = 0; j < matSize - 1; j++) {
            //System.out.println(exceptionIndex.get(j) + " " + Arrays.toString(occurences[j]));
            System.out.println(Arrays.stream(occurences[j]).mapToObj(x -> x > 0 ? String.valueOf(x) : "").collect(Collectors.joining(" & ")));
        }
        System.out.println(Arrays.stream(occurences[matSize - 1]).mapToObj(String::valueOf).collect(Collectors.joining(" & ")));
        //System.out.println("Successful " + Arrays.toString(occurences[matSize - 1]));
        ...
        relevantExceptions.stream().sorted().forEach(ex -> {
            ...
            System.out.println(ex + " " + problem.getSearchSpace().getSearchSpace().stream().mapToInt(y -> y.getKey().length).sum() + " | " + problem.getSearchSpace().getSearchSpace().stream().mapToInt(x -> x.getValue().length).sum());
        });
        System.out.println("Successful: " + successfulSolutions.getSearchSpace().getSearchSpace().stream().mapToInt(x -> x.getKey().length).sum() + " | " + successfulSolutions.getSearchSpace().getSearchSpace().stream().mapToInt(x -> x.getValue().length).sum());
...
//                    }
    ...
    private static String process(String exception) {
        ...
        if (ex.contains("An illegal reflective access operation has occurred")) {
            return "An illegal reflective access operation has occurred";
        } else if (ex.contains("The worker crashed")) {
            return "The worker crashed";
        }

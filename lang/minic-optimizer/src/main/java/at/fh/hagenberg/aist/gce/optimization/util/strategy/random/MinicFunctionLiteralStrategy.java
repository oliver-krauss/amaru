...
public class MinicFunctionLiteralStrategy extends RandomReflectiveSubtreeStrategy {

    public MinicFunctionLiteralStrategy(MinicContext minicContext) {
    ...
    private static Map<String, TruffleVerifyingStrategy> loadTerminalStrategy(MinicContext minicContext) {
        ...
        List<String> functionNames = minicContext.getFunctionRegistry().getFunctions().stream().map(MinicFunctionNode::getName).filter(x -> !x.equals("read") && !x.equals("main") &&  !x.endsWith("_benchmark") && !x.endsWith("_entry")).collect(Collectors.toList());

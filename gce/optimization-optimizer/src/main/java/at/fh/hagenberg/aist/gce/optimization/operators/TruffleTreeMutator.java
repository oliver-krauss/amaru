 ...
 */
public class TruffleTreeMutator implements ConfigurableMutator<TruffleOptimizationSolution, TruffleOptimizationProblem> {
    ...
    private Node attemptMutation(Node mutatedNode, CreationConfiguration configuration, TruffleLanguageContextProvider context, TruffleFunctionSignature signature) {
        ...
        Node mutationNode = selector.selectSubtree(mutatedNode);
        ...
        if (mutationNode.getParent() == null) {
            ...
            if (maxWidth > 10) {
                ...
                // chance for random reduction
                maxWidth = random.nextInt(maxWidth - 2) + 3;
            } else if (maxWidth < 1) {
                maxWidth = 1;
            }

 ...
 */
public class StringSubgraphIterator implements SubgraphIterator {

    private Logger logger = new Logger();

    private static final int SUBTREE_OPEN = -128;
    private static final int SUBTREE_CLOSE = -127;
    ...
    /**
     * Determines if values of nodes should be ignored or not.
     */
    ...
    public StringSubgraphIterator(TrufflePatternSearchSpace searchSpace, int maxPatternSize, boolean explicit) {
    ...
    public PatternNodeWrapper next() {
        ...
        if (subgraphs.size() > 0) {
            return (deString(subgraphs.remove(0)));
        }
     ...
     */
    private PatternNodeWrapper deString(int[] s) {
        try {
            PatternNodeWrapper result = copy(nodes[s[0]]);
            ...
            for (int i = 1; i < s.length; i++) {
                ...
                } else if (s[i] == SUBTREE_CLOSE) {
                    currentParent = parent.pop();
                    current = currentParent;
                } else {
                    current = copy(nodes[s[i]]);
                    if (currentParent != null) {
                        Long parentId = currentParent.getId();
                        Long childId = current.getId();
                        OrderedRelationship rel = Arrays.stream(relationshipInfo).filter(x -> x.getParent().getId().equals(parentId) && x.getChild().getId().equals(childId)).findFirst().orElse(null);
     ...
     */
    private PatternNodeWrapper copy(NodeWrapper wrapper) {
        if (explicit) {
            return new PatternNodeWrapper(wrapper, wrapper.getId());
        } else {
            PatternNodeWrapper patternNodeWrapper = new PatternNodeWrapper(new NodeWrapper(wrapper.getType()), wrapper.getId());

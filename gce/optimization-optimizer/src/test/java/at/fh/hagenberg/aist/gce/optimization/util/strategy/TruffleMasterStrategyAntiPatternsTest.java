 ...
 */
public class TruffleMasterStrategyAntiPatternsTest extends AbstractPatternsTest {
     ...
     */
    @Test
    public void testInjectAntiPattern() {
        // given
     ...
     */
    @Test
    public void testInjectAntiPatternHierarchy() {
        // given
     ...
     */
    @Test
    public void testInjectAntiPatternWithValueRequirement() {
        ...
        create().injectAntiPattern(antiPatternDos, datatypeIndependentMeta);
     ...
     */
    @Test
    public void testInjectAntiPatternWithStarValueReq() {
        List<String> datatypes = new ArrayList<>();
        datatypes.add("Char");
        datatypes.add("Int");
        datatypes.add("Float");
...
//                    System.out.println(child.getValues().entrySet());
...
//                }
    ...
    public void testNChildPatternWithChildren() {
        ...
        create().injectAntiPattern(antiPattern, meta);
        create().injectAntiPattern(patternInterferingNodes(), meta);
        create().injectAntiPattern(new NodeWrapper(MinicWhileNode.class.getName() + "|" + MinicForNode.class.getName()), meta);
    ...
    private String createAST(NodeWrapper wrap, int depth, int width) {
        String construct = "";
        Object[] objects = wrap.getChildren().toArray();
        String children = "";
        for (int i = 0; i < wrap.getChildren().size(); i++) {
            children += "node" + (depth + 1) + "_" + i;
            if (i < wrap.getChildren().size() - 1) {
                children += ", ";
            }
            construct += createAST(((OrderedRelationship) objects[i]).getChild(), depth + 1, i) + "\n";
        }
        construct += wrap.getType().substring(wrap.getType().lastIndexOf(".") + 1) + " node" + depth + "_" + width + " = new " + wrap.getType().substring(wrap.getType().lastIndexOf(".") + 1) + "(" + children + ");";
        return construct;
    }
    ...
    public void testChildWithChildPattern() {
        ...
        create().injectAntiPattern(new NodeWrapper(MinicRepeatingNode.class.getName()), meta);
        create().injectAntiPattern(new NodeWrapper(MinicWhileNode.class.getName() + "|" + MinicForNode.class.getName()), meta);
    ...
    /**
     * Tests root not pattern - we essentially enforce that any mutation will be done with "double + double"
     * Tests non-array child-creation and n children together
     */
    @Test
    public void testRootNotPattern() {
        // given
        // antipattern - enforces that Block Node consists of IntLit, DoubleLit, CharLit
        NodeWrapper antiPattern = new NodeWrapper(Wildcard.WILDCARD_NOT + MinicDoubleArithmeticNode.MinicDoubleAddNode.class.getName());
        NodeWrapper leftLitNode = new NodeWrapper(Wildcard.WILDCARD_NOT + MinicSimpleLiteralNode.MinicDoubleLiteralNode.class.getName());
        NodeWrapper rightLitNode = new NodeWrapper(Wildcard.WILDCARD_NOT + MinicSimpleLiteralNode.MinicDoubleLiteralNode.class.getName());
    ...
    public void testRootStarwildcardPattern() {
        ...
        // when
        create().injectAntiPattern(starWildcard, meta);
    ...
    public void testWhilePattern() {
        ...
        create().injectAntiPattern(new NodeWrapper(MinicFunctionBodyNode.class.getName()), meta);

...
array nn_sigmoid(int numTrainingSets, float output[], float training_inputs[][], float training_outputs[][]) {
    ...
    for (n = 0; n < 1000; n = n + 1) {
        ...
        for (x = 0; x < numTrainingSets; x = x + 1) {
            ...
            for (j = 0; j < numHiddenNodes; j = j + 1) {
                ...
                hiddenLayer[j] = 1 / (1 + exp(-activation));
            }
            for (j = 0; j < numOutputs; j = j + 1) {
                ...
                outputLayer[j] = 1 / (1 + exp(-activation));
            }
...
array nn_entry(int numTrainingSets, float training_inputs_raw[], float training_outputs_raw[]) {
    ...
    nn_sigmoid(numTrainingSets, output, training_inputs, training_outputs);

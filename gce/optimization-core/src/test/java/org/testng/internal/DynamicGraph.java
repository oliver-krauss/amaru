...
public class DynamicGraph<T> implements IDynamicGraph<T> {
    ...
    public List<T> getFreeNodes() {
        ...
        // Filter down to the last class that was free
        // this is the fix for TestNGs mind-bogglingly inane decision to ignore sequences on Before/After Method
        // See https://maven.apache.org/surefire/maven-surefire-plugin/examples/fork-options-and-parallel-execution.html
        //
        if (finalResult.size() > 0 && finalResult.get(0) instanceof ITestNGMethod) {
            List<ITestNGMethod> results = new ArrayList<ITestNGMethod>((Collection<? extends ITestNGMethod>) finalResult);
            if (!results.stream().anyMatch(x -> x.getRealClass().equals(lastFreeClass))) {
                // find next class with possible edges
                lastFreeClass = results.get(0).getRealClass();
            }
            results.stream().filter(x -> !x.getRealClass().equals(lastFreeClass)).forEach(finalResult::remove);
        }

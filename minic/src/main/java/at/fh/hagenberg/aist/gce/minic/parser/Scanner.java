...
public class Scanner {
	...
	static {
		...
		for (int i = 48; i <= 57; ++i) start.set(i, 14);
		start.set(39, 6); 
		start.set(34, 9); 
		for (int i = 43; i <= 43; ++i) start.set(i, 11);
		for (int i = 45; i <= 45; ++i) start.set(i, 11);
		for (int i = 37; i <= 37; ++i) start.set(i, 12);
		for (int i = 42; i <= 42; ++i) start.set(i, 12);
		for (int i = 47; i <= 47; ++i) start.set(i, 12);
		start.set(61, 30); 
		start.set(33, 31); 
		start.set(62, 15); 
		start.set(60, 16); 
		...
		literals.put("const", new Integer(9));
		literals.put("struct", new Integer(12));
		literals.put("void", new Integer(18));
		literals.put("while", new Integer(21));
		literals.put("for", new Integer(22));
		literals.put("return", new Integer(23));
		literals.put("if", new Integer(24));
	...
	Token NextToken() {
		...
		loop: for (;;) {
			switch (state) {
				...
				case 6:
					if (ch >= 9 && ch <= 10 || ch == 13 || ch == ' ' || ch == 39 || ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch == 92 || ch >= 'a' && ch <= 'z') {AddCh(); state = 7; break;}
					else {state = 0; break;}
				case 7:
				...
				case 8:
					{t.kind = 4; break loop;}
				case 9:
					if (ch >= 9 && ch <= 10 || ch == 13 || ch == ' ' || ch == 39 || ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch == 92 || ch >= 'a' && ch <= 'z') {AddCh(); state = 9; break;}

...
array nn_options(int numTrainingSets, float output[], float training_inputs[][], float training_outputs[][]) {
    ...
    for (n = 0; n < 1000; n = n + 1) {
        ...
        for (x = 0; x < numTrainingSets; x = x + 1) {
            ...
            for (j = 0; j < numHiddenNodes; j = j + 1) {
                ...
                hiddenLayer[j] = actFn(activation);
            }
            for (j = 0; j < numOutputs; j = j + 1) {
                ...
                outputLayer[j] = actFn(activation);
            }
            ...
            for (j = 0; j < numOutputs; j = j + 1) {
                ...
                deltaOutput[j] = errorOutput * dActFn(outputLayer[j]);
            }
            ...
            for (j = 0; j < numHiddenNodes; j = j + 1) {
                ...
                deltaHidden[j] = errorHidden * dActFn(hiddenLayer[j]);
            }
...
array nn_entry(int numTrainingSets, float training_inputs_raw[], float training_outputs_raw[]) {
    ...
    nn_options(numTrainingSets, output, training_inputs, training_outputs);
...
int main() {
    ...
    float training_inputs[8];
    training_inputs[0] = 0.0; training_inputs[1] = 0.0;
    training_inputs[2] = 1.0; training_inputs[3] = 0.0;
    training_inputs[4] = 0.0; training_inputs[5] = 1.0;
    training_inputs[6] = 1.0; training_inputs[7] = 1.0;

    float training_outputs[numTrainingSets];
    training_outputs[0] = 0.0;
    training_outputs[1] = 1.0;
    training_outputs[2] = 1.0;
    training_outputs[3] = 0.0;

    nn_entry(numTrainingSets, training_inputs, training_outputs);
